export const UNIVERSAL_SIGNALS = [
...
export function isTypeMismatch(assignedType, suggestedType) {
...
export function validateCustomSignals(customSignals) {
    const errors = [];
    ...
    for (const signal of customSignals) {
        ...
        if (RESERVED_KEYS.has(signal.key) || seenKeys.has(signal.key)) {
